export default {
  "title": "FuelFinder Italy - Real Prices",
  "loader_h2": "Loading ministerial data...",
  "loader_p": "This operation may take a few seconds...",
  "badge_dati": "Real Ministerial Data",
  "badge_solo": "Italy Only 🇮🇹",
  "lbl_location": "ZIP Code / City",
  "ph_location": "E.g. 00100 or Rome (Italian locations only)",
  "btn_search": "Search",
  "title_gps": "Use GPS",
  "lbl_radius": "Distance",
  "lbl_fuel": "Fuel Type",
  "fuel_gasoline": "Petrol",
  "fuel_diesel": "Diesel",
  "fuel_lpg": "LPG",
  "fuel_methane": "CNG (Methane)",
  "lbl_service": "Service",
  "service_self": "Self Service",
  "service_served": "Served",
  "service_both": "Both",
  "status_ready": "Data is ready. Search, use GPS, or <strong class=\"text-blue-700 dark:text-blue-400\">click directly on the map</strong>.",
  "rp_title": "Recommended Choice",
  "rp_best_badge": "Best",
  "rp_selected_title": "Route & Price",
  "rp_station_badge": "Station",
  "rp_brand": "Brand:",
  "rp_price": "Price:",
  "rp_dist": "📍 Distance:",
  "rp_time": "⏱️ Estimated time:",
  "btn_share": "Share Station",
  "share_copied": "Link Copied!",
  "btn_close": "Close Panel",
  "footer_text": "Data provided by <a href=\"https://www.mimit.gov.it/it/\" target=\"_blank\" rel=\"noopener noreferrer\" class=\"font-bold hover:text-blue-700 dark:hover:text-blue-400 transition-colors\">MIMIT</a> (Free Open Data)",
  "table_title": "<span>🏆</span> Best Prices Ranking",
  "th_pos": "Pos.",
  "th_brand": "Gas Station",
  "th_address": "Address",
  "th_dist": "Distance",
  "th_price": "Price",
  "dyn_error_load": "<div class=\"text-red-600 text-xl font-bold\">Data loading error! Check the 'data' folder.</div>",
  "dyn_not_found": "Location not found.",
  "dyn_enter_valid": "Enter a valid ZIP code or city.",
  "dyn_gps_fetching": "Acquiring location...",
  "dyn_gps_error": "Unable to get GPS location.",
  "dyn_current_pos": "Current Location",
  "dyn_map_point": "Point on map",
  "dyn_start_point": "<div class='font-bold text-center p-1 text-slate-800 dark:text-white'>Starting Point</div>",
  "dyn_no_stations": "No gas stations found in the selected radius.",
  "dyn_found": "Found",
  "dyn_stations": "gas stations.",
  "dyn_showing": "Showing the",
  "dyn_best_fluidity": "most convenient on map for fluidity",
  "dyn_best_price": "Best price in the area:",
  "dyn_results": "results",
  "view_map": "Map",
  "view_list": "List",
  "view_both": "Both",
  "your_position": "Your position",
  "price_label": "Price:",
  "admin_title": "📊 Admin Dashboard",
  "admin_passkey": "Enter Passkey:",
  "admin_error": "Invalid passkey or access denied",
  "admin_btn_access": "Access Statistics",
  "admin_btn_back": "← Back to Main Site",
  "admin_stats_title": "Recorded Statistics",
  "admin_btn_exit": "Lock & Exit",
  "admin_no_stats": "No statistics found. The stats.json file is empty.",
  "admin_th_date": "Date",
  "admin_th_visits": "Total Visits",
  "admin_th_unique": "Unique Visitors",
  "admin_th_searches": "Total Searches",
  "fuel_hvo": "HVO (Bio-Diesel)",
  "fuel_gnl": "LNG (Trucks)",
  "loading_app": "Loading...",
  "map_prompt": "Click on the map or use search to begin",
  "sidebar_location": "Current Location",
  "sidebar_nav": "Navigation",
  "sidebar_home": "Home (Map)",
  "sidebar_stats": "Price Trends",
  "sidebar_explore": "Explore Cities (Directory)",
  "sidebar_install": "Install on Mobile (QR)",
  "install_modal_title": "📱 Install FuelFinder",
  "install_modal_subtitle": "Scan the QR code with your phone camera to open and install the app.",
  "install_ios_title": "🍎 On iPhone (Safari)",
  "install_ios_step": "Tap the 'Share' icon and select 'Add to Home Screen'.",
  "install_android_title": "🤖 On Android (Chrome)",
  "install_android_step": "Tap menu (⋮) and select 'Install app' or 'Add to Home Screen'.",
  "install_copy_link": "Copy Share Link",
  "install_link_copied": "Link Copied! ✅",
  "get_directions": "Get directions",
  "open_menu": "Open Menu",
  "close_menu": "Close menu",
  "italy": "Italy",
  "cities": {
    "roma": "Rome",
    "milano": "Milan",
    "napoli": "Naples",
    "venezia": "Venice",
    "firenze": "Florence",
    "torino": "Turin",
    "genova": "Genoa",
    "padova": "Padua",
    "siracusa": "Syracuse",
    "mantova": "Mantua"
  }
}
